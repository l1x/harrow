...
| Tokio Hyper prototype | `harrow-server-tokio-hyper` planned | Not available yet | Prototype candidate for stable 1.0 Tokio path | Linux, macOS, Windows |
...
| --- | --- | --- | --- | --- |
| HTTP/1.1 | Public today; Harrow-owned protocol stack | Planned | Public today; Harrow-owned protocol stack | Experimental |
| Custom Harrow H1 path | Yes | No; Hyper owns protocol | Yes | Yes |
| HTTP/2 | Planned for 1.0; not implemented yet | Planned/preferred evaluation path | Partial/backend-local implementation and tests; needs stabilization | Planned for parity before stabilization |
...
| Graceful shutdown | Yes | Required for prototype | Yes | Yes, experimental |
| Multi-worker mode | Yes | Required, including thread-per-core profile | Yes | Yes |
...
Before the 1.0 backend policy is finalized, Harrow should add a
...
- Tokio Hyper prototype is planned specifically to evaluate whether Hyper should own Tokio HTTP/1+HTTP/2 protocol handling for the stable path.
