...
COPY --from=build-env /app/target/aarch64-unknown-linux-gnu/perf/harrow-perf-server /
CMD ["/harrow-perf-server", "--bind", "0.0.0.0"]

# --- axum-perf-server ---
FROM gcr.io/distroless/cc-debian13:latest-arm64 AS axum-perf-server
COPY --from=build-env /app/target/aarch64-unknown-linux-gnu/perf/axum-perf-server /
