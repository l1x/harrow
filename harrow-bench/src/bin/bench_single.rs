...
use harrow_bench::harness::runner::{CommonRunConfig, SingleRunConfig, run_single};
use harrow_bench::harness::spec::DeploymentMode;

fn usage() -> ! {
    eprintln!(
        "Usage: bench-single --impl ID --suite PATH [OPTIONS]\n\
         \n\
         Required:\n\
         \x20 --impl ID                 Implementation id from harrow-bench/implementations.toml\n\
...
fn main() {
    ...
    let mut implementation_id: Option<String> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--impl" => {
                implementation_id = Some(args.get(i + 1).unwrap_or_else(|| usage()).clone());
                i += 2;
            }
            "--suite" => {
    ...
    let implementation_id = implementation_id.unwrap_or_else(|| usage());
    if !common.suite_path.exists() {
        eprintln!("suite file not found: {}", common.suite_path.display());
        std::process::exit(1);
    }

    match run_single(SingleRunConfig {
        common,
        implementation_id,
    }) {
        Ok(_) => {}
